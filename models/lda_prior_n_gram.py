...
class LDA_NGramModel:
    ...
    def predict_next_word(self, context, doc_id):
        ...
        word_ids = np.array([self.lda_model.word_to_id.get(word, -1) for word in self.ngram_model.vocabulary])
        valid_indices = word_ids >= 0
        word_ids = word_ids[valid_indices]
        
        word_topic_probs = np.dot(topic_probs, self.topic_word_dist[:, word_ids])
        ngram_probs = np.array([(word_counts.get(word, 0) + self.ngram_model.alpha) / 
                                (total_count + self.ngram_model.alpha * len(self.ngram_model.vocabulary)) 
                                for word in self.ngram_model.vocabulary])[valid_indices]
        ...
        return np.random.choice(np.array(list(self.ngram_model.vocabulary))[valid_indices], p=final_probs)
...
if __name__ == "__main__":
    file_path = 'conversation_datasets/cleaned_files/cleaned_dailydialog.txt'
    ...
    ngram_model = NGramMarkovChain(n=3, alpha=0.0)
